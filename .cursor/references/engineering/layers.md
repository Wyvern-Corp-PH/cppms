...
Physical trees vary by stack; keep a durable **folder blueprint** for *this* repo — see [`folder-structure.md`](./folder-structure.md) and the always-on [`folder-structure`](../../rules/folder-structure.mdc) rule. The sections below show common layer folder names, not a mandatory monorepo layout.
