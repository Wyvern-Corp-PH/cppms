...
- **User Management** (`/users`) — Super Admin only (V115–V121)
  ...
  - Province role policies: can Update Progress, approve/reject/request revision + process fund releases + project mutate; ⊥ manage users; ⊥ system settings; ⊥ Activity Logs.
...
V93: Approvals detail panel: name, location, category, lgu_level, status, contractor, timeline, description; budget summary total/spent(red)/savings(green); progress history w/ transitions, notes, photos, updated_by; V13 warning + Super Admin/Province Approve/Reject/Request Revision CTAs for Ready for Review entries only (V4,V109,V158); For Revision shows latest revision note and no Province/SA action CTAs until Barangay resubmits.
...
V119: Province policy grants system-wide project read/mutate, Progress Update Progress, approval actions, and fund release processing; ⊥ manage users/system settings/Activity Logs.
...
V158: Approval actions (Approve, Reject, Request Revision) available to Super Admin and Province/Provincial Admin; Barangay/Municipality users see status/read-only detail only; backend rejects non–Super Admin/Province approval mutations.
...
V167: Approval UI fail-closed: absent/unknown/non–Super Admin/Province actor sees status/read-only detail only; Approve/Reject/Request Revision buttons ⊥.
...
V169: RBAC action matrix (SoT): Super Admin — Users nav/manage, Project mutate, Budget allocate + Released Amount, Progress Update Progress (no embedded Released Amount), Approvals actions, Activity Logs; Province — same except Users + Activity Logs hidden; Municipality/Barangay — Progress Update Progress scoped + embedded Released Amount required; Project/Budget allocate/Released Amount CTA/Approvals actions/Users/Activity Logs hidden.
...
V205: Role-scope checks distinguish project mutation from progress mutation: Municipality/Barangay scoped actors may `progress_updates.create` only for in-scope projects, while `projects.create|update|delete` stays denied and Projects UI hides project mutate affordances; Super Admin + Province may also `progress_updates.create|update|delete` (action matrix) without embedded Released Amount.
